...
include('./bdg/includes/httproot.php');



// NOTE: your phpslideshow.php script will work "out of the box" and 
// all layout and visual effects are controlled by the template.php file.
...
// sort images with newest or oldest on top or alphabetically by name.
...
//	$header = "PHPSlideshow by zinkwazi.com";
	$title = "$header";
}
else {
...
function my_filmstrip($thumbnail_dir, &$template, $a_images, $currentPic, $thumb_row, $directory) {
    global $path;
    global $auto_url;
    ...
    for ($x = $start; $x < ($start + $thumb_row); $x++ ) {
        ...
        $out .= "\n<a href='$path?directory=$directory$auto_url&currentPic=$x' class='thumbnail'><img src='$directory/$thumbnail_dir/".$item[0]."' class='$class'></a>";
    }
...
if( file_exists( "$directory/$thumbnail_dir" ) ) {
    if( $thumbnail_view == "circular" ) {
        ...
        my_filmstrip($thumbnail_dir, $template, $pic_info, $currentPic, $thumb_row, $directory); 
    }
}
